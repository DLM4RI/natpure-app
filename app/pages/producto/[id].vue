<template>
  <v-container v-if="producto" class="py-8">
    ...
    <v-row>
      ...
      <v-col cols="12" md="6" class="d-flex flex-column justify-center pl-md-8">
        <div
          class="mb-2 text-overline text-medium-emphasis text-uppercase tracking-wide"
        >
          {{ producto.categoria }}
        </div>
        ...
        <div class="d-flex align-center mb-6">
          <span class="text-subtitle-1 font-weight-medium mr-4 text-primary"
            >Cantidad:</span
          >
          <v-btn
            icon
    ...
    <v-row>
      ...
      >
        <tarjeta
          :producto="sugerencia"
          @agregar-carrito="alAgregarSugerenciaCarrito"
        ></tarjeta>
      </v-col>
    </v-row>
  </v-container>
  ...
  <v-container v-else class="py-16 text-center">
    <InfoCircle color="medium-emphasis" class="mb-4" style="font-size: 4rem;" />
...
import { ArrowBack, Minus, Plus, Cart, InfoCircle } from "@boxicons/vue";
...
const alAgregarCarrito = () => {
  if (producto.value) {
    ...
    productosStore.agregarAlCarrito({
      ...producto.value,
